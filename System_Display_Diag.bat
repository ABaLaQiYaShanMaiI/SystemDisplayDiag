...
:: ===== 2. PLATFORM TYPE (Win10/11 - PowerShell, no wmic) =====
echo ============================================================ >> "%REPORT%"
...
powershell -NoProfile -Command ^
  "Get-CimInstance Win32_Processor | ForEach-Object { Write-Output ('Name=' + $_.Name); Write-Output ('Cores=' + $_.NumberOfCores); Write-Output ('Logical=' + $_.NumberOfLogicalProcessors); Write-Output ('MaxClock=' + $_.MaxClockSpeed) }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"

echo --- Motherboard --- >> "%REPORT%"
powershell -NoProfile -Command ^
  "Get-CimInstance Win32_BaseBoard | ForEach-Object { Write-Output ('Mfr=' + $_.Manufacturer); Write-Output ('Product=' + $_.Product); Write-Output ('Version=' + $_.Version); Write-Output ('SN=' + $_.SerialNumber) }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"

echo --- Physical Memory --- >> "%REPORT%"
powershell -NoProfile -Command ^
  "Get-CimInstance Win32_PhysicalMemory | ForEach-Object { Write-Output ('Bank=' + $_.BankLabel); Write-Output ('CapacityGB=' + ([math]::Round($_.Capacity/1GB,1))); Write-Output ('Speed=' + $_.Speed); Write-Output ('Mfr=' + $_.Manufacturer); Write-Output ('PartNo=' + $_.PartNumber); Write-Output '---' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"
...
:: ===== 4. GRAPHICS ADAPTER DETAILS (ENHANCED) =====
...
powershell -NoProfile -Command ^
  ...
  "if ($conn) { " ^
  "  $techMap = @{0='VGA';1='S-Video';2='Composite';3='Component';4='DVI';5='HDMI';6='LVDS(eDP)';8='DisplayPort';9='8P9C_Digital';10='DisplayPort(e)'}; " ^
  "  foreach ($c in $conn) { $tech = if ($techMap.ContainsKey($c.VideoOutputTechnology)) { $techMap[$c.VideoOutputTechnology] } else { 'Unknown(' + $c.VideoOutputTechnology + ')' }; Write-Output ('Monitor: ' + $c.InstanceName); Write-Output ('  ConnectionType: ' + $tech) } " ^
  "} else { Write-Output 'WmiMonitorConnectionParams not available.' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"
echo --- CurrentScanMode (GPU-level) --- >> "%REPORT%"
powershell -NoProfile -Command ^
...
powershell -NoProfile -Command ^
  ...
  "if ($m) { foreach ($i in $m) { Write-Output ('Monitor: ' + $i.InstanceName); Write-Output ('  PhysicalSize(cm): ' + $i.MaxHorizontalImageSize + ' x ' + $i.MaxVerticalImageSize); Write-Output ('  TransferChar: ' + $i.DisplayTransferCharacteristic) } } else { Write-Output 'Not found.' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"
echo --- Raw EDID (Hex) --- >> "%REPORT%"
powershell -NoProfile -Command ^
  "$edid = Get-CimInstance -Namespace root/WMI -ClassName WmiMonitorDescriptorMethods; " ^
  "if ($edid) { foreach ($e in $edid) { $r = Invoke-CimMethod -InputObject $e -MethodName WmiGetMonitorRawEEdidV1Block -Arguments @{BlockId=0} -ErrorAction SilentlyContinue; if ($r -and $r.ReturnValue -eq 0) { Write-Output ([System.BitConverter]::ToString($r.BlockContent) -replace '-','') } else { Write-Output 'Not retrievable.' } } } else { Write-Output 'No EDID methods.' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"
...
echo ============================================================ >> "%REPORT%"
powershell -NoProfile -Command ^
  "$b = Get-CimInstance -Namespace root/WMI -ClassName WmiMonitorBrightness -ErrorAction SilentlyContinue; if ($b) { foreach ($item in $b) { Write-Output ('Current Brightness: ' + $item.CurrentBrightness) } } else { Write-Output 'WmiMonitorBrightness not found.' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"

:: ===== 8. INTEL HD GRAPHICS REGISTRY =====
echo ============================================================ >> "%REPORT%"
...
echo. >> "%REPORT%"
echo --- DPST (Display Power Saving Technology) --- >> "%REPORT%"
...
:: ===== 11. SYSTEM EVENTS (ENHANCED: Display/PNP/TDR) =====
echo ============================================================ >> "%REPORT%"
echo   SECTION 11: SYSTEM EVENTS (DISPLAY/PNP/TDR) >> "%REPORT%"
echo ============================================================ >> "%REPORT%"
powershell -NoProfile -Command ^
  "$events = Get-WinEvent -LogName System -MaxEvents 200 -ErrorAction SilentlyContinue | Where-Object { $_.Id -in @(4101,4109,4110,4114,5010,5011) -or $_.ProviderName -in @('Microsoft-Windows-Kernel-PnP','Display') }; " ^
  "if ($events) { $i = 0; foreach ($evt in $events) { Write-Output ("Event[$i]:"); Write-Output ("  Source: " + $evt.ProviderName); Write-Output ("  Date: " + $evt.TimeCreated); Write-Output ("  EventID: " + $evt.Id); Write-Output ("  Level: " + $evt.LevelDisplayName); $desc = $evt.Message; if ($desc) { Write-Output ("  Desc: " + $desc.Substring(0, [Math]::Min(300, $desc.Length))) }; Write-Output ''; $i++ } } else { Write-Output 'No relevant display/PNP/TDR events found in last 200 system events.' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"
...
if "!TYPE!"=="laptop" (
    ...
    powershell -NoProfile -Command ^
      "$w = Get-CimInstance -Namespace root/WMI -ClassName WmiMonitorBrightnessMethods -ErrorAction SilentlyContinue; if ($w) { Write-Output 'Available' } else { Write-Output 'Not found' }" >> "%REPORT%" 2>&1
    echo. >> "%REPORT%"
...
echo ============================================================ >> "%REPORT%"
echo   SECTION 14: DEVICE MANAGER PROBLEM DEVICES >> "%REPORT%"
echo ============================================================ >> "%REPORT%"
powershell -NoProfile -Command ^
  "$err = Get-CimInstance Win32_PnPEntity | Where-Object { $_.ConfigManagerErrorCode -ne 0 }; " ^
  "if ($err) { " ^
  "  $codes = @{1='Not configured';2='Disabled';3='Driver error';9='BIOS error';10='Failed start';12='No driver';14='Service fail';18='Reinstall';19='Registry corrupt';21='Removing';22='Disabled(user)';24='Not present';28='No driver';29='Disabled(fw)';31='Load fail';32='Removed'}; " ^
  "  foreach ($d in $err) { $msg = if ($codes[$d.ConfigManagerErrorCode]) { $codes[$d.ConfigManagerErrorCode] } else { 'Code ' + $d.ConfigManagerErrorCode }; Write-Output ("Device: $($d.Name)"); Write-Output ("  Error: $msg ($($d.ConfigManagerErrorCode))"); Write-Output ("  PNPID: $($d.PNPDeviceID)"); Write-Output '---' } " ^
...
echo ============================================================ >> "%REPORT%"
echo --- Windows Forms Screen Info --- >> "%REPORT%"
powershell -NoProfile -Command ^
  "Add-Type -AssemblyName System.Windows.Forms; " ^
  "$screens = [System.Windows.Forms.Screen]::AllScreens; " ^
  "Write-Output ("Monitor count: $($screens.Count)"); " ^
  "foreach ($s in $screens) { Write-Output ("  Device: $($s.DeviceName)"); Write-Output ("  Primary: $($s.Primary)"); Write-Output ("  Resolution: $($s.Bounds.Width)x$($s.Bounds.Height)"); Write-Output ("  Position: ($($s.Bounds.X),$($s.Bounds.Y))"); Write-Output ("  BitsPerPixel: $($s.BitsPerPixel)"); Write-Output '' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"
echo --- Monitor ID (WMI) --- >> "%REPORT%"
powershell -NoProfile -Command ^
  "$ids = Get-CimInstance -Namespace root/WMI -ClassName WmiMonitorID -ErrorAction SilentlyContinue; " ^
  "if ($ids) { foreach ($m in $ids) { $name = ($m.UserFriendlyName -ne 0 | ForEach-Object { [char]$_ }) -join ''; $sn = ($m.SerialNumberID -ne 0 | ForEach-Object { [char]$_ }) -join ''; Write-Output ("Monitor: $name"); Write-Output ("  Serial: $sn"); Write-Output ("  Week: $($m.WeekOfManufacture)"); Write-Output ("  Year: $($m.YearOfManufacture)"); Write-Output '---' } } else { Write-Output 'WmiMonitorID not available.' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"
...
powershell -NoProfile -Command ^
  ...
  "if ($tdr) { Write-Output ("Driver crash count (Event 4101): $($tdr.Count)"); $tdr | Select-Object -First 10 | ForEach-Object { Write-Output ("  Time: $($_.TimeCreated) - $($_.Message.Substring(0,[Math]::Min(200,$_.Message.Length)))") } } else { Write-Output 'No display driver crash events (4101) found.' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"
echo --- GPU Hung/Reset Events --- >> "%REPORT%"
powershell -NoProfile -Command ^
  "$extra = Get-WinEvent -LogName System -MaxEvents 500 -ErrorAction SilentlyContinue | Where-Object { $_.Id -in @(4109,4110,4114,5010,5011) }; " ^
  "if ($extra) { Write-Output ("Additional GPU events: $($extra.Count)"); $extra | Select-Object -First 10 | ForEach-Object { Write-Output ("  ID=$($_.Id) Time=$($_.TimeCreated)") } } else { Write-Output 'No additional GPU events found.' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"
...
echo ============================================================ >> "%REPORT%"
echo --- WDDM VRR --- >> "%REPORT%"
powershell -NoProfile -Command ^
  "$vrr = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\Dwm' -Name 'DisableVRROverlay' -ErrorAction SilentlyContinue; if ($vrr) { Write-Output ("DisableVRROverlay: $($vrr.DisableVRROverlay)") } else { Write-Output 'VRR registry key not found.' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"
echo --- NVIDIA G-Sync --- >> "%REPORT%"
reg query "HKLM\SOFTWARE\NVIDIA Corporation\Global\GSync" /s >> "%REPORT%" 2>&1
echo. >> "%REPORT%"

:: ===== 20. GRAPHICS PERFORMANCE SETTINGS =====
echo ============================================================ >> "%REPORT%"
echo   SECTION 20: GRAPHICS PERFORMANCE >> "%REPORT%"
echo ============================================================ >> "%REPORT%"
echo --- HAGS (Hardware-Accelerated GPU Scheduling) --- >> "%REPORT%"
...
echo ============================================================ >> "%REPORT%"
powershell -NoProfile -Command ^
  "@('NVDisplay.ContainerLocalSystem','DisplayEnhancementService','AMD External Events Utility','igfxCUIService2.0.0.0') | ForEach-Object { Write-Output ("--- $_ ---"); sc.exe query $_ >> "%REPORT%" 2>&1; Write-Output '' }" >> "%REPORT%" 2>&1
echo. >> "%REPORT%"

:: ===== 25. DISPLAY SOFTWARE (COMPREHENSIVE) =====
echo ============================================================ >> "%REPORT%"
